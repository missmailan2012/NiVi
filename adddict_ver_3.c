...
int main()
{
  ...
  printf("\t5\tTính từ な.\n");
  printf("\t6\tDanh từ.\n");
  printf("\t7\tCâu nói trong các tình huống.\n");
  fflush(stdin);
  scanf("%d",&tempi);
  switch(tempi)
    {
    ...
    case 6:output[temp+5]=100; break;
    case 7:output[temp+6]=99; break;
    }
  temp+=10;
  system("clear");
  ...
  output[temp-5]='\'';
  strcpy(&output[temp-4],echo3);
  system(output);
  system("clear");
  printf("Đã thêm từ vào cơ sở dữ liệu.\n");
  return 0;
}
